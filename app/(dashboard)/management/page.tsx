  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 border-b border-slate-100 pb-8">
        <div>
          <h2 className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#6b4f3f] mb-1">Workshop</h2>
          <h1 className="font-serif italic text-4xl text-[#2e2e2e]">My Creations</h1>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-[#faf7f2]/30">
            ...
            <p className="font-serif italic text-lg text-slate-500">Your workshop is currently empty.</p>
            <Link href="/management/new" className="mt-4 text-[#c97c5d] text-sm font-bold uppercase tracking-widest hover:underline">
              Start creating
            </Link>
          </div>
        ) : (
          <ul className="divide-y divide-slate-50">
            {products.map((p: SellerProduct) => (
              <li key={p.id} className="group flex flex-col sm:flex-row items-center gap-6 p-6 hover:bg-[#faf7f2]/50 transition-colors">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                    <h3 className="font-serif text-xl text-[#2e2e2e]">{p.name}</h3>
                  ...
                  <div className="flex items-center justify-center sm:justify-start gap-4 mt-2">
                    <p className="text-lg font-light text-[#6b4f3f]">${Number(p.price).toFixed(2)}</p>

                    <div className="flex items-center gap-1 text-[#c97c5d]">
