    ...
    <main className="mx-auto max-w-7xl px-4 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-4 mb-8">
            <span className="text-3xl font-light text-[#2e2e2e]">
      ...
      <section className="mt-20 max-w-3xl">
          ...
          {reviews.length === 0 ? (
            <div className="bg-[#faf7f2] p-8 rounded-2xl text-center border border-dashed border-slate-200">
              <p className="text-slate-500 italic text-sm">
                Be the first to value this masterpiece.
              </p>
            </div>
          ) : (
            reviews.map((r) => (
              <div
                key={r.id}
                className="relative pl-6 border-l-2 border-[#c97c5d]/20"
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <StarIcon
                        key={i}
                        className={`w-3 h-3 ${i < Number(r.rating) ? "text-[#c97c5d]" : "text-slate-200"}`}
                      />
                    ))}
                  </div>
                </div>
        ...
        <div className="mt-16 bg-[#faf7f2] rounded-3xl p-8 border border-slate-100">
          <h3 className="font-serif italic text-xl text-[#2e2e2e] mb-2">
            Leave your mark
          </h3>
          <p className="text-xs text-slate-500 mb-6 uppercase tracking-wider">
            Share your thoughts with the creator
          </p>
